...
def grad_descent(X, y, reg=0.0, lr=1e-4, eps=1e-6, max_iter=500, print_freq=20):
 ...
 #checked solution:
		...
		if (iter_num + 1) % print_freq == 0:print('-- Iteration {} - negative log likelihood {: 4.4f}'.format(\
         iter_num + 1, nll))
...
def get_description(X, y, W):
	...
	"*** YOUR CODE HERE ***"
    #checked solution. Was very confused for this part
	m, n = X.shape
	y_pred = predict(X, W)
...
def plot_description(X_train, y_train, X_test, y_test):
    ...
    # saw solution to see I needed reg_list
